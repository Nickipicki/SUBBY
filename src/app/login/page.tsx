...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen flex bg-[#0B0F19] relative overflow-hidden justify-center gap-32">
      ...
      {/* Auth Switch */}
      <div className="hidden lg:flex absolute left-1/2 top-[15%] -translate-x-1/2 z-10">
        <AuthSwitch />
      </div>

      {/* Mittlerer Auswahlslider */}
      <div className="hidden lg:flex absolute left-1/2 top-[15%] -translate-x-1/2 z-10">
        <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-1">
          <div className="relative flex space-x-1">
            <div className="absolute bg-gradient-to-r from-purple-500 to-indigo-500 h-full w-1/2 rounded-xl transition-all duration-300 left-0" />
            <Link
              href="/login"
              className="relative px-8 py-3 rounded-xl text-white font-medium z-10"
            >
              Anmelden
            </Link>
            <Link
              href="/register"
              className="relative px-8 py-3 rounded-xl text-gray-400 font-medium hover:text-white transition-colors"
            >
              Registrieren
            </Link>
          </div>
      ...
      <motion.div 
        className="w-full lg:w-[600px] flex items-center justify-start relative"
        initial={{ x: "100%", opacity: 0, scale: 0.8 }}
        animate={{ x: 0, opacity: 1, scale: 1 }}
        exit={{ x: "-100%", opacity: 0, scale: 0.8 }}
        transition={{
